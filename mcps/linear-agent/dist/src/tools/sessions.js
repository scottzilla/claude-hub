...
export function registerSessionTools(server) {
    server.registerTool("linear_create_session", {
        ...
        const data = await gql(CREATE_SESSION_MUTATION, { issueId: args.issueId });
        return { content: [{ type: "text", text: JSON.stringify(data.agentSessionCreateOnIssue.agentSession, null, 2) }] };
    });
    server.registerTool("linear_update_session", {
        ...
        if (args.externalUrls)
            input.externalUrls = args.externalUrls;
        if (args.plan)
            input.plan = args.plan;
        const data = await gql(UPDATE_SESSION_MUTATION, { id: args.sessionId, input });
        return { content: [{ type: "text", text: JSON.stringify(data.agentSessionUpdate.agentSession, null, 2) }] };
    });
    server.registerTool("linear_create_activity", {
        ...
        const data = await gql(CREATE_ACTIVITY_MUTATION, { input });
        return { content: [{ type: "text", text: JSON.stringify(data.agentActivityCreate.agentActivity, null, 2) }] };
    });
}
