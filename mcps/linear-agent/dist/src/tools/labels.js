...
export function registerLabelTools(server) {
    server.registerTool("linear_list_labels", {
        ...
        return { content: [{ type: "text", text: JSON.stringify(data.issueLabels.nodes, null, 2) }] };
    });
    server.registerTool("linear_create_label", {
        ...
        const input = { teamId: args.teamId, name: args.name };
        if (args.color)
            input.color = args.color;
        if (args.parentId)
            input.parentId = args.parentId;
        const data = await gql(CREATE_LABEL_MUTATION, { input });
        return { content: [{ type: "text", text: JSON.stringify(data.issueLabelCreate.issueLabel, null, 2) }] };
    });
}
