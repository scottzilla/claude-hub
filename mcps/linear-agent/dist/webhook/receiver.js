...
function verifySignature(body, signature) {
    if (!SECRET)
        return false; // No secret = reject all webhooks
    if (!signature)
        return false;
...
`;
async function ackSession(sessionId, message = "Starting up...") {
    try {
...
async function spawnClaude(event) {
    if (!TARGET_REPO) {
    ...
    const promptBody = activity?.body;
