# ScottClip Design Spec

**Date:** 2026-03-25
**Status:** Draft
**Author:** Alex Kim + Claude
**Linear:** [WOT-79](https://linear.app/wotai/issue/WOT-79/design-scottclip-agent-orchestration-system)
...
ScottClip is a Claude Code plugin that provides Linear-backed agent orchestration with persona-based task routing. It replaces Paperclip's agent system by using Linear as the task store and Claude Code's `/schedule` as the heartbeat runner.
...
```
scottclip/
  package.json                # Plugin manifest (name: "scottclip")
  commands/
    heartbeat.md              # /heartbeat slash command
    scottclip-init.md         # /scottclip-init slash command
    scottclip-status.md       # /scottclip-status slash command
  skills/
    heartbeat/
      SKILL.md                # Core heartbeat procedure
    init/
      SKILL.md                # Initialize ScottClip in a repo
    persona-create/
  ...
  references/
    ...
    status-mapping.md         # Linear states <-> ScottClip states
  templates/
...
**Per-repo scaffolding** (created by `/scottclip-init`):
...
1. **Load Config** — Read `.scottclip/config.yaml`. Verify Linear MCP is available. Check for lockfile (`.scottclip/.heartbeat-lock`). If lockfile exists and is less than `stale_lock_hours` old, skip this heartbeat ("previous heartbeat still active"). If stale, delete lockfile and proceed. Create lockfile with current timestamp. Delete lockfile on exit (including error paths).
...
7. **Understand Context** — Read issue title, description, comments. Read parent issue if exists. Check for new comments since last heartbeat. Parse the heartbeat counter from the last ScottClip comment (look for `Heartbeat #N` pattern). If no previous comment, start at #1.
...
9. **Report** — Post structured comment on Linear issue via `mcp__claude_ai_Linear__save_comment`. Include heartbeat counter (incremented from step 7). Store heartbeat metadata to `.scottclip/heartbeat-log.jsonl` (timestamp, issue ID, persona, duration, status, heartbeat number) for `/scottclip-status --history`.
...
The heartbeat counter is **derived from Linear comments**, not stored locally. On each heartbeat, parse the last ScottClip comment on the issue for `Heartbeat #N` and increment. If comments are deleted, the counter resets — this is acceptable as the counter is informational, not functional.

### Overlap Prevention

A lockfile at `.scottclip/.heartbeat-lock` prevents concurrent heartbeats. Created at step 1, deleted at step 11 (and in error handlers). Stale lockfiles (older than `stale_lock_hours`) are auto-cleaned.
...
| Paperclip File | ScottClip Equivalent | Purpose |
|----------------|---------------------|---------|
...
5. Create "ScottClip" label group in Linear, then create persona labels (e.g., `backend`, `frontend`, `ceo`) as child labels
...
---
*ScottClip · backend · [WOT-79](link) · from [Heartbeat #2](link)*
```
...
1. Install ScottClip plugin from marketplace
2. Run `/scottclip-init` in any repo
...
The `/persona-import` skill converts Paperclip agent directories into ScottClip persona directories.

### Mapping

| Paperclip File | ScottClip Target | What's Imported |
|----------------|-----------------|-----------------|
...
- **Budget tracking** — Paperclip budget fields have no ScottClip equivalent
...
Each heartbeat appends a JSON line to `.scottclip/heartbeat-log.jsonl`:
...
This powers `/scottclip-status --history`. The file is append-only and can be safely truncated (informational, not functional).

### Lockfile

`.scottclip/.heartbeat-lock` contains the timestamp when the current heartbeat started. Auto-deleted on exit. Stale locks (older than `stale_lock_hours`) are auto-cleaned.
...
| Capability | Paperclip | ScottClip |
|-----------|-----------|-----------|
