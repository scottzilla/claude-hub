...
Convert a Paperclip agent directory into a ScottClip persona. Maps Paperclip's agent structure (SOUL.md, AGENTS.md, HEARTBEAT.md, TOOLS.md) to ScottClip's persona structure (SOUL.md, TOOLS.md, config.yaml).

## Prerequisites

- `.scottclip/config.yaml` must exist (run `/scottclip-init` first)
- User must provide the path to the Paperclip agent directory

## File Mapping

| Paperclip File | ScottClip Target | What to Import |
|----------------|-----------------|----------------|
...
4. Add a ScottClip-specific "## Quality Checklist" section if not present
5. Drop any Paperclip-specific references (workspace paths, budget tracking, approval workflows). Keep `para-memory-files` and `qmd` references — ScottClip uses the same memory skill.
...
2. Replace Paperclip API references:
   ...
   - `para-memory-files` / `qmd query` → keep as-is (ScottClip includes the `para-memory-files` skill)
...
1. Create `.scottclip/personas/<label>/`
2. Write `SOUL.md`, `TOOLS.md`, `config.yaml`
3. Update `.scottclip/config.yaml` to add the persona to the `personas` map
...
Files created:
  ✓ .scottclip/personas/<label>/SOUL.md
  ✓ .scottclip/personas/<label>/TOOLS.md
  ✓ .scottclip/personas/<label>/config.yaml
...
What was NOT imported:
  ✗ Budget tracking (no ScottClip equivalent)
  ✗ Approval workflows (not in v1)
  ✓ PARA memory — carried over (ScottClip includes para-memory-files skill)
